...
async def get_overview(
    ...
    total_action_items_result = await db.execute(select(func.count(ActionItem.id)))
    ...
    overdue_result = await db.execute(
        select(func.count(ActionItem.id)).where(ActionItem.status == ItemStatus.overdue)
    )
    ...
    critical_result = await db.execute(
        select(func.count(ActionItem.id)).where(
            ...
            ActionItem.status != ItemStatus.completed
        )
    )
